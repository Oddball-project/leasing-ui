export function getChartColorsArray(colors) {
    ...
    return colors.map(function (value) {
        ...
        if (newValue.indexOf(",") === -1) {
            var color = typeof window !== 'undefined' && typeof window !== 'undefined' && getComputedStyle(document.documentElement).getPropertyValue(newValue);
            if (color) {
                color = color.replace(" ", "");
                return color;
            }
            ...
            var val = value.split(',');
            if (val.length == 2) {
                var rgbaColor = typeof window !== 'undefined' && typeof window !== 'undefined' && getComputedStyle(document.documentElement).getPropertyValue(val[0]);
